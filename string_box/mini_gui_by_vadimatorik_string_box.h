...
struct mini_gui_by_vadimatorik_lcd_mono_string_box_cfg_t {
    ...
    const uint8_t  width_string_box_pixel;      // Ширина в пикселях.
    const uint8_t  height_string_box_pixel;     // Высота в пикселях.
    const uint8_t   width_line;                 // Ширина окаймления линии. Находится внутри бара.
    const uint8_t	thickness_slider;           // Толщина ползунка.
    const uint8_t	x_pos;                      // Позиция левого верхнего угла бара.
    const uint8_t 	y_pos;

    const uint8_t	string_number;              // Колличество строк в списке.
    char**          string_name;                // Массив, состоящий из указателей на строки. Т.к. строк может быть string_number. Создается пользователем (имеет произвольный размер).
    char**          string_time;                // Массив, состоящий из указателей на время каждого трека (каждая строка - 6 char-ов).
